...
function run(action, curRow, verbose) {
  ...
  var url = "http://enhancer.sdsc.edu/ysun/t2dream/";
...
function selTestServer() {
  var server_name = "https://test.t2dream-demo.org";
...
function selEncodeServer() {
  var server_name = "http://www.t2dream-demo.org";
  PropertiesService.getUserProperties().setProperty("submit_server", server_name);
  Browser.msgBox("Use: "+server_name);
}
...
function onOpen() {
  ...
  var ui = SpreadsheetApp.getUi();
  var menu = ui.createMenu('T2DREAM');

  var server_name=PropertiesService.getUserProperties().getProperty("submit_server");
  if (server_name==null) {
    server_name="http://www.t2dream-demo.org";
    PropertiesService.getUserProperties().setProperty("submit_server", server_name);
  }
  var userProperties = PropertiesService.getUserProperties();
  ...
  if (userProperties.getProperty(server_name) != null) {
    menu.addSubMenu(ui.createMenu('Selected')
        .addItem("Get from T2DREAM", "doGet")
        .addItem("Post to T2DREAM", "doPost")
        .addItem("Patch to T2DREAM", "doPatch")
        )
      .addSubMenu(ui.createMenu('ALL')
        .addItem("Get all from T2DREAM", "doGetAll")
        .addItem("Post all to T2DREAM", "doPostAll")
        .addItem("Patch all to T2DREAM", "doPatchAll")
        )
      .addSeparator()
      .addSubMenu(ui.createMenu('Settings')
        .addSubMenu(ui.createMenu('Select servers')
          .addItem("Test server", "selTestServer")
          .addItem("T2DREAM server", "selT2DREAMServer")
        )
